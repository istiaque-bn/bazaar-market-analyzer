...
class TickerJsonReadOnlyTests(TestCase):
    """The public ticker endpoint must serve cached/DB state only — it must
    never start a background sync thread or honor a public refresh param
    (previously: every request, including anonymous ones, spawned a
    threading.Thread running maybe_sync(), and `?refresh=1` forced it)."""
