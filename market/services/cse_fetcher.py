...
def _session(verify=None):
    ...
    # CSE TLS chain often fails on local Python installs; prefer reliable fetch.
    s.verify = False if verify is None else verify
    try:
        import urllib3
...
def fetch_cse_live_via_bdshare() -> pd.DataFrame | None:
    try:
        from market.services.dse_fetcher import _ensure_ssl_patch
...
def sync_cse_live() -> dict:
    ...
    if df is not None and not df.empty:
        count = upsert_live_quotes(df, Exchange.CSE)
