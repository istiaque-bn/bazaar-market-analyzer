...
def compute_economic_diagnostics(rows: list[dict], family: str) -> dict:
    ...
    def _cohort_means(values: np.ndarray) -> np.ndarray:
        return np.array([
            float(np.mean([values[i] for i, key in enumerate(cohort_keys) if key == cohort_key]))
            for cohort_key in unique_keys
        ])

    benchmark_period_returns = _cohort_means(np.array([float(r["outcome_return"]) for r in ordered]))

    def _summary(multiplier: float) -> dict:
        per_row_net = np.where(is_long, gross - round_trip_cost_frac * multiplier, 0.0)
        net = _cohort_means(per_row_net)
        ...
        return {
            ...
            "estimated_total_cost_pct": round(float(sum(
                np.mean([is_long[i] for i, key in enumerate(cohort_keys) if key == cohort_key])
                for cohort_key in unique_keys
            )) * round_trip_cost_frac * multiplier * 100, 4),
        }

    benchmark_equity = np.cumprod(1 + benchmark_period_returns)
    benchmark_total_return_pct = round((float(benchmark_equity[-1]) - 1) * 100, 4) if len(benchmark_equity) else 0.0
    at_1x = _summary(1.0)

    return {
        "n": n,
        "cohort_count": len(unique_keys),
        "policy": (
